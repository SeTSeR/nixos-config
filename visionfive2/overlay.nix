self: super:
let disableTests = pkg: pkg.overrideAttrs (self: super: { doCheck = false; });
in {
  libpng = super.libpng.overrideAttrs (finalAttrs: prevAttrs: {
    configureFlags = [ "--disable-riscv-rvv" "--enable-apng" ];
  });
  libsecret = disableTests super.libsecret;
  ffmpeg = super.ffmpeg.override { withSdl2 = false; };
  tailscale = super.tailscale.overrideAttrs (self: super: {
